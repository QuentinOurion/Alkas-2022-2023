 ...
 * @return array
 */
function findUserByLogin(PDO $connexion, string $login): array
{
    return find($connexion, '
            select *
            from user
            where login = :login', [
        'login' => $login
 ...
 * @return bool
 */
function addUser(PDO $connexion, int $roleId, string $login, string $password, string $prenom, string $nom, string $dateNaissance): bool
{
    $prepare = $connexion->prepare('insert into user (roles_id, login, password, prenom, nom, date_naissance, date_inscription) values (:roles_id, :login, :pwd, :prenom, :nom, :date_naissance, now())');
